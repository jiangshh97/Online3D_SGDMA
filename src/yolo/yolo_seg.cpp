...
void YOLO::postprocess(std::vector<Object> &objs, float score_thres, float iou_thres, int topk, int seg_channels, int seg_h, int seg_w)
{
    ...
    if (masks.empty() || protos.empty())
    {
        //        std::cerr << "Error: 'masks' or 'protos' is empty." << std::endl;
        return;
    }
    else
    {
...
void YOLO::draw_objects(const cv::Mat &image, cv::Mat &res, const std::vector<Object> &objs,
                        ...
                        const std::vector<std::vector<unsigned int>> &MASK_COLORS)
{
    ...
    for (auto &obj : objs)
    {
        ...
        char text[256];
        sprintf(text, "%s %.2f%", CLASS_NAMES[obj.label].c_str(), obj.prob);
        ...
        cv::putText(res, text, cv::Point(x, y + label_size.height), cv::FONT_HERSHEY_SIMPLEX, 0.7, {255, 255, 255}, 1.5);
